...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function detectLang() {
      ...
      const host = (document.documentElement.getAttribute('lang') || '').trim().toLowerCase()
      const nav = (navigator.language || '').trim().toLowerCase()
      const probe = host || nav
      if (probe.indexOf('zh') === 0) return 'zh'
      if (probe.indexOf('en') === 0) return 'en'
      return 'zh'
    }

    /** Diagnostics: where the current language came from, so `auto` is inspectable. */
    function langSource() {
      const pref = settings().lang
      if (pref === 'zh' || pref === 'en') return 'setting=' + pref
      const host = (document.documentElement.getAttribute('lang') || '').trim()
      if (host) return 'html[lang]=' + host
