...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    .loupe-ws-item.on{background:rgba(51,78,172,.16);font-weight:600}
    .loupe-ghost{background:transparent;color:inherit;border:1px solid rgba(8,31,92,.2)}
    ...
    function workspaceChoices(ctx) {
      try {
        ...
        const snapshot = workspaces && typeof workspaces.getSnapshot === 'function' ? workspaces.getSnapshot() : null
    ...
    function commitSave(parts, transcript, title, choice) {
      ...
      const workspaces = clientCtx && typeof clientCtx.get === 'function' ? clientCtx.get('workspaces') : null
      ...
      sessions
        ...
        .then((sessionId) => {
          savedId = sessionId
          // 关键补上的一步：sessions.create 只把会话写进磁盘（并按 cwd 落目录），
          // **不会把它登记进工作区的会话列表** —— 所以左侧列表里看不到它。
          // 登记要走 workspace-controller 的 insertSessionBefore（省略 anchor 即追加）。
          if (choice === null || choice === undefined || choice.id === undefined) return null
          if (workspaces === null || workspaces === undefined) return null
          if (typeof workspaces.insertSessionBefore !== 'function') return null
          return workspaces.insertSessionBefore(choice.id, sessionId).catch(() => null)
        })
        .then((registration) => {
          if (registration && registration.ok === false) {
            const reason = registration.error && registration.error.message
              ? registration.error.message
              : 'unknown'
            parts.body.appendChild(el('div', 'loupe-note', '会话已建，但没能登记进工作区：' + reason))
          }
