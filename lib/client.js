...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    .loupe-ws{margin-top:10px;padding:9px;border-radius:8px;border:1px solid rgba(8,31,92,.14);background:rgba(186,214,235,.16)}
    .loupe-ws-title{font-size:12.5px;font-weight:600;color:#334eac;margin-bottom:6px}
    .loupe-ws-item{all:unset;display:block;width:100%;box-sizing:border-box;cursor:pointer;padding:6px 8px;border-radius:6px;font-size:12.5px;color:#334eac;word-break:break-all;margin-bottom:2px}
    .loupe-ws-item:hover{background:rgba(51,78,172,.12)}
    .loupe-ws-item.none{opacity:.7}
    ...
    function saveCurrent() {
      ...
      // 不再推测工作区（DOM 探测和列表快照都试过，两次都落错地方）：
      // 把候选项列出来，让用户自己选。
      const sessions = clientCtx && typeof clientCtx.get === 'function' ? clientCtx.get('sessions') : null
      renderWorkspacePicker(parts, collectWorkspaces(sessions), (cwd) => {
        commitSave(parts, transcript, title, cwd)
      })
    }

    /**
     * 候选工作区 = 会话列表里出现过的 cwd，按会话数降序。
     * 一个目录下会话越多，越可能是用户当前在用的项目。
     */
    function collectWorkspaces(sessions) {
      ...
      try {
        ...
        const snapshot = list !== null && list !== undefined && typeof list.getSnapshot === 'function'
          ? list.getSnapshot()
          : null
        const byId = snapshot !== null && snapshot !== undefined && snapshot.byId ? snapshot.byId : null
        if (byId !== null) {
      ...
      const out = []
      for (const entry of counts.entries()) out.push({ cwd: entry[0], count: entry[1] })
      out.sort((a, b) => b.count - a.count)
      return out
    }

    /** 保存目标选择器：点哪个就存到哪个。 */
    function renderWorkspacePicker(parts, workspaces, onPick) {
      ...
      if (workspaces.length === 0) {
        box.appendChild(el('div', 'loupe-param-hint', '（会话列表里没有带工作目录的会话）'))
      }
      for (const ws of workspaces) {
        const row = el('button', 'loupe-ws-item')
        row.textContent = ws.cwd
        row.title = ws.count + ' 个会话'
      ...
      const none = el('button', 'loupe-ws-item none')
      none.textContent = '不指定（未分组）'
    ...
    /** 用户选定目标后真正落库：客户端建档 + 投递。 */
    function commitSave(parts, transcript, title, cwd) {
      ...
      sessions
        ...
        .then((sessionId) => {
          ...
          // content 必须是 ContentBlock[]：prompt 实现里有
          // `content.some(part => part.type === 'file')`。
          return bound.session.prompt([{ type: 'text', text: transcript }], 'queue').then(() => sessionId)
        })
        .then((sessionId) => {
          succeed('已保存到 ' + (cwd === undefined ? '未分组' : cwd) + '（' + String(sessionId).slice(0, 22) + '…），模型会先接一句。')
        })
        .catch(fail)
    }
