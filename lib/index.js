...
function buildUserText({ selection, question, messageText, neighborCount, neighbors }) {
  const parts = []
  if (messageText) {
    parts.push(`【这段话所在的对话内容】\n${messageText}`)
  }
  if (neighborCount > 0 && Array.isArray(neighbors) && neighbors.length > 0) {
    const rendered = neighbors
      ...
      .join('\n')
    parts.push(`【前后对话片段，共 ${neighbors.length} 条】\n${rendered}`)
  }
  parts.push(`【选中文字】\n${selection}`)
  parts.push(question)
  return parts.join('\n\n')
...
function createChunkReader() {
  ...
  return {
    read(chunk) {
      ...
      return {
        text: isAnswer ? extractText(chunk) : '',
...
function handleHistory(req, res) {
  ...
  try {
    ...
    if (raw !== null) limit = Math.max(1, Math.min(500, Number(raw) || 100))
  } catch {
    /* keep the default */
  }
...
async function handleExplain(ctx, req, res, cfg) {
  ...
  const question = clamp(String(body.question ?? '').trim() || '请解释下面这段内容。', MAX_QUESTION_CHARS)
  ...
  const system = buildSystem(contextClamped.text)
  ...
  try {
    ...
    appendHistory({ ...historyBase, ok: true, answer, ...summary })
  } catch (error) {
