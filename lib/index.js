...
function hostLocalePreference(ctx) {
  // 首选宿主 settings 服务：它不关心底层存储。2026-09-25 实测新版 DSH 会把
  // settings.yaml 迁移走（本机那个文件已不存在），直接读文件会读空。
  try {
    const settings = ctx && typeof ctx.get === 'function' ? ctx.get('settings') : null
    if (settings && typeof settings.get === 'function') {
      const section = settings.get('locale')
      if (section && typeof section.preference === 'string' && section.preference.length > 0) {
        return { preference: section.preference, source: 'settings service' }
      }
    }
  } catch {
    /* 服务不可用就退回旧的文件解析 */
...
function resolveModel(ctx, cfg) {
  ...
  try {
    ...
    if (svc === undefined || svc === null) {
      ...
      // 读取方法名以宿主实现为准。@deepseek-ai/dsh-agent-default-model 的
      // AgentDefaultModelConfig 只提供 currentSelection()（返回一份独立的
      // { provider, model, reasoningEffort? }）与 saveSelection()。
      // 2026-09-25 的 issue #1：候选表里漏了 currentSelection，于是这一档只剩
      // self 兜底，而 cordis Service 对象的键永远是 { ctx, name, ownerContext,
      // config } —— 必然落空；同日 settings.yaml 又被新版 DSH 迁移走，第 3 档
      // 也失效，两档同时断掉 → /explain 一律 503。
      const readKeys = ['currentSelection', 'snapshot', 'current', 'get', 'value', 'read']
      const candidates = []
      ...
      const present = readKeys.filter((key) => typeof svc[key] === 'function')
      tried.push(`service agentDefaultModel: present but no {provider,model} shape (read methods: ${present.join(',') || 'none'}; own keys: ${Object.keys(svc).slice(0, 12).join(',')})`)
    }
  } catch (error) {
    tried.push(`service agentDefaultModel: ${error?.message ?? error}`)
  }

  // 3. settings.yaml —— 旧版位置，仅作兜底。新版 DSH 把默认模型存在 profile 的
  // patch 里、由上面的 agentDefaultModel 服务读出来；本机 2026-09-25 实测该文件
  // 已被迁移走（只剩 settings.yaml.imported），所以这一档正常就是 ENOENT。
  for (const file of settingsCandidates()) {
    try {
      const raw = fs.readFileSync(file, 'utf8')
...
function handleProbe(ctx, res, cfg) {
  ...
  sendJson(res, 200, {
    ...
    locale: hostLocalePreference(ctx),
    loupeDir: loupeDir(),
    context: {
      file: context.file,
...
function handleLocale(ctx, res) {
  const found = hostLocalePreference(ctx)
  sendJson(res, 200, {
    ok: true,
